...
> ~~**C-1** 出勤日数が3画面で食い違う~~ → **2026-09-22 対応済み**。
> 「出勤日数＝出勤・退勤とも打刻が揃っている日」に3画面を統一（有給日・退勤未打刻日は除外）。
> あわせて「有給取得」の表示を件数から日数換算（半休0.5日・時間休0.25日）に変更し、
> 有給残日数の計算と同じ `leaveDays()` を使うようにした。
> **残る論点：`leaveDays()` は振替休日を年次有給休暇1日として消化扱いにしている**（C-2）。
>
> ~~**D-3** 祝日リストが 2026-11-23 までしかない~~ → **2026-09-22 対応済み**。
> 内閣府の公開CSVから2027年末まで登録した（2025-11-23 の抜けも同時に修正）。
> **2028年分が公表されたら追記が必要**。index.html の `HOLIDAYS` にソースURLを記載。

- **C-2** 有給残日数：時間休が一律0.25日（実時間を見ていない）／失効した付与分の控除が残り続ける／申請時の残日数・重複日チェックなし
  （消化日数の判定は `leaveDays()` に集約済み）

  > ~~振替休日を年休として消化計上~~ → **2026-09-22 対応済み**。`leaveDays("振替休日")` を 0 にし、
  > 振替元の日（`workDate`）を記録・表示するようにした。

### 振替休日の仕様（2026-09-22 実装）
- `leaves` の `type:"振替休日"` に `workDate`（出勤する休日＝振替元）を追加。`date` は休む日（振替先）
- 年次有給休暇の残日数は消化しない
- 月次タブ：振替休日の行に「🔄 振替休日 ─ 07/04 の振替」、休日出勤の行に「休日出勤」／「休日出勤・振替済」を表示
- 事後登録：月次タブの「⚠️ 未打刻」行の 🔄 ボタンから、その日を振替休日として申請できる
- **未対応**：代休（事後に休みを与える形）との区別。代休は休日割増が必要になるため、
  厳密な給与計算が要るなら別種別として追加する必要がある（労働基準法第37条第1項）
- **未対応**：管理者の勤怠確認タブには休日出勤バッジを出していない（admin.html に祝日リストが無いため）
