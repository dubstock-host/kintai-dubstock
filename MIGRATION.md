...
**管理者画面の「👤 社員管理」タブから登録します**（2026-09-26〜）。
氏名・雇用形態・メール・初期パスワード・入社日（パートは週の勤務日数と時間）を入れて「登録する」。
Authentication のアカウントと `users/{uid}` が同時に作られます。Firebase コンソールでの作業は不要です。

退職時は同じタブの「退職にする」。ログインが止まり、過去の記録は管理者から見られます。
